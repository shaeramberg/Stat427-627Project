...
We understand the choice of the authors to use an A-B-C subset method, however, given time constraint, we decided to solely select subset (C). Not including G1 or G2 would indicate a model that can predict a students grade for trimester 3 wihtout considering trimester 1 and 2.
...
Note: we will use cross validation (as requested in the project instructions) and 20% testing 80% training data.
...
library(dplyr)
lrpca <- glm(pass_fail ~ . -five_level -G3 -G2 -G1, data = portuguese.data, 
          family = binomial) 
```
...
Very clearly were one variable carrying the most weight, so scaling is better here. We will now view the variance of our pca's to decide how many to include in our model:
...
portguese_pcr <- pcr(pass_fail ~ . -five_level -G3 -G2 -G1, 
...
Based on the plot displaying our RMSEP, points at around 16, 22, and 35 components should give us the lowest RMSEP. When looking at the summary in depth, 21 and 22 components have the same low .3394 RMSEP for CV with 21 components at .3380 for adjusted CV. This RMSEP doesn't get much smaller later on with more components, with 35 components having a .3378 RMSEP. Given this, to avoid over fitting, we will select 21 components for our final model. Noting, this is not a great RMSEP rate, as it does not explain a lot of the variance.
...
Our accuracy rate was actually pretty good, with our model accurately predicting 84.2% of our data correctly. I also added a matrix to look at specifically what the model is doing well or poorly. It accurately categorized most of the passing students correctly, and seems to inaccurately predict those who failed the class overall, with 68 of those who failed being classified by the model as passed, versus 16 correct predictions of those who failed. Overall, our model seems to do well at predicting if you passed, when you actually do pass. It is not the best at predicting the failing students. Note: inaccurately predicted 80.9% of the failing students to passing.
...
portuguese.glm <- glm(pass_fail ~ . -G3 -five_level -G2 -G1, 
                      data = portuguese.data_01)
X <- model.matrix(portuguese.glm) [,-1]

set.seed(627)
portuguese_lasso <- cv.glmnet(x = X, y = portuguese.data_01$pass_fail, 
                              alpha = 1,
                              family = "binomial")
portuguese_lasso
```

I see that our model for the minimum cv error (which was 0.02119) is selecting 21 predictors, the same amount of components we selected in our PCA model. For 1se, it selected 9 predictors with a 0.06472 error rate. Both of these are actually really good error rates for our model. For our model, I would like to select the min model with a 21 of our predictors. Its error rate is far smaller, and the deviance is lower. Lets see which of the predictors were selected.
...
mean(classification == portuguese.data_01$pass_fail)
```

It predicted 84.97% of the training data correctly, which is also relatively good. There are 9 nonzero predictors. For caution, we will produce a model that displays error changes across differing lambda values:
...
set.seed(627) 
library(tree) 
tree_initial<-tree(pass_fail ~ . -G3-five_level-G2-G1, data=portuguese.data) 
...
The initial tree is very busy and could possibly overfit our data, so we will now tune the tree.
...
set.seed(627)
randomForest(pass_fail ~ . -G3-five_level-G2-G1, data=portuguese.data, mtry=6, importance=TRUE)->RF6
RF6
randomForest(pass_fail ~ . -G3-five_level-G2-G1, data=portuguese.data, mtry=5, importance=TRUE)->RF5
RF5
set.seed(627)
randomForest(pass_fail ~ . -G3-five_level-G2-G1, data=portuguese.data, mtry=4, importance=TRUE)->RF4
RF4
randomForest(pass_fail ~ . -G3-five_level-G2-G1, data=portuguese.data, mtry=7, importance=TRUE)->RF7
RF7
```

Looking at the OOB estimate of error rate, and keeping in mind the ideal of m = sqrt(p), we tried making m 4, 5, 6, and 7 (we have 30 predictors in the model). The model with m=8 produces the smallest OOB error rate of 15.41.
...
We were curious if changing the number of predictors considered at each branch to be the predictor amounts favored in previous models, like 21 and 9, would give better results:

```{r}
set.seed(627)
randomForest(pass_fail ~ . -G3-five_level-G2-G1, data=portuguese.data, mtry=21, importance=TRUE, ntree = )->RF21

randomForest(pass_fail ~ . -G3-five_level-G2-G1, data=portuguese.data, mtry=9, importance=TRUE)->RF9

RF21
RF9
```

Our error rate is 16.76%, but we have a relatively better prediction ability for failing students, 10% improvement. We will keep the model with the smaller OOB error rate.

### Model 5: Support Vector Machine

```{r}
library(e1071)
svm.lin<-svm(pass_fail ~ . -G3-five_level-G2-G1, kernel="linear", data=portuguese.data)
summary(svm.lin)
svm.pol<-svm(pass_fail ~ . -G3-five_level-G2-G1, kernel="polynomial", data=portuguese.data)
summary(svm.pol)
svm.rad<-svm(pass_fail ~ . -G3-five_level-G2-G1, kernel="radial", data=portuguese.data)
summary(svm.rad)
svm.sig<-svm(pass_fail ~ . -G3-five_level-G2-G1, kernel="sigmoid", data=portuguese.data)
summary(svm.sig)
...
svm.tuning1 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = 0.001, kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning1

svm.tuning2 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = 0.01, kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning2

svm.tuning3 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = 0.1, kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning3

svm.tuning4 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = 1, kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning4

svm.tuning5 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = 10, kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning5

svm.tuning6 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = 100, kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning6

svm.tuning7 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = 1000, kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning7

#Below is the previous tuning method
#svm.tuning1 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = c(0.001, 0.01, 0.1, 1, 10, 100, 1000), kernel = c("linear", "polynomial", "radial", "sigmoid")))
#svm.tuning1
#svm.tuning1$best.parameters

```

The cost of 1000 takes more computing time so it is run in a separate code chunk.

```{r}

svm.tuning7 <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = 1000, kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning7
svm.tuning7$best.parameters
```

```{r}
svm.tuning5$best.parameters
best.svm<-svm(pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, cost=10, kernel="radial")
best.svm
```
...
Our testing data did even better than our model on the training data, we received 90.8% of accurate predictions, as well as improved our prediction ability for those who actually failed the class, with our previous classification of those who failed being falsely labeled as passed at 80.6%, and we are now at 68.75%. I would say this model accurately predicts our data.

#### Lasso Regression - Binary Setup C

```{r}
portuguese.glm.holdout <- glm(pass_fail ~ . -G3 -five_level -G2 -G1, 
                      data = portuguese.data_01_holdout)
...
predictions <- predict(portuguese_lasso, newx = X, s = "lambda.min", type = "response")
classification <- ifelse(predictions > 0.5, 1, 0)
mean(classification == portuguese.data_01_holdout$pass_fail)
```

The testing data was better predicted then the training data, in comparison to 83.81% prediction accuracy. This model also reflects predictive ability well.
...
Our initial tree model had a misclassification rate of around 13.08% on our testing data. We will tune it to see if that can be improved. Tuning the tree with cross validation brings our misclassification rate down to 11.54% which is better, but the model is more inflexible now. Note that reason is a categorical variable with 4 options, course and other being two of the options.
...
```{r}
svm.pred <- predict(best.svm, newdata=holdout.data)
err.tab <- c(mean(holdout.data$pass_fail != svm.pred), svm.tuning6$best.performance)
err.tab
```

Change the below? Where did it come from?

Misclassification rate on new data: 13.08%

Misclassification based on CV for the training data: 13.50%
