...
In summary, we intend to replicate this study, with key differences: we intend to use solely setup (C), we intend to replicate Decision Tree and Random Forest from the original paper but replace the other models with... change

Partial Least Squares, Lasso and LDA, and examine solely the Portuguese course data for time constraint reasons.

# Part 2: Model Fitting - Binary; Set Up C

If time allocates, we still intend to work towards replicating the models with five-level as well.
...
#Commented out code that changed pass fail to 0 1 because our models need to read the response as classification (because they are classification models)
...
Note: we will use cross validation (as requested in the doc) and 20% testing 80% training data.
...
Our error is pretty high, given such, we will tune our tree further, such as using predictor amounts used in previous models, like 21 and 9:
...
```{r}
svm.tuning1$best.parameters
best.svm<-svm(pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, cost=100, kernel="linear")
best.svm
```
...
#err.tab <- c(mean(holdout.data$pass_fail != svm.pred), svm.tuning$best.performance)
#err.tab
```
