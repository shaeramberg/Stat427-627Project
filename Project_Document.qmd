...
Run the following before modeling and use Portuguese.data to ensure all variables are there and read as the correct class (potentially delete pass/fail or five/level if we commit to using not those)

PSA: Do not do any testing data matricies or calculations yet, wait until all models are complete before using the testing data at the end.
...
Also note we will use cross validation (as requested in the doc) and 20% testing 80% training data.

Re-do the below LR using PCA

## Logistic Regression (LR) with PCA - Binary Outcome- CHANGE
