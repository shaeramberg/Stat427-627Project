...
```{r}
set.seed(627)
svm.tuning <- tune(svm, pass_fail ~ . , data = portuguese.data_01_scaled, ranges = list(cost = c(0.01, 0.1, 1, 10, 100), kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning
svm.tuning$best.parameters
svm.tuning$best.model
```

The best model has a cost of 1, uses the radial kernel, and has a CV prediction error of 12.23%.
...
The best kernel for cost 100 is radial with an error of 12.74%.

The best kernel for cost 10 is radial with an error of 12.57%.

The best kernel for cost 1 is radial with an error of 12.23%.

The best kernel for cost 0.1 is linear with an error of 13.37%.

The best kernel for cost 0.01 is linear with an error of 14.13%.

```{r}
best.svm<-svm(pass_fail ~ ., data = portuguese.data_scaled, cost=1, kernel="radial")
best.svm
```
...
Misclassification rate on new data: 10.77%.
...
|------------|------------|------------|------------|------------|------------|
| Our Misclassification Error Rate    | 10.77%                   | 9.23%                    | 11.54% | 12.3% | 10.77%   |
| Papers Misclassification Error Rate | **Not in initial paper** | **Not in initial paper** | 15.6%  | 15%   | 15.2%    |
| Our best Models Rank                | 2 (tied)                 | 1                        | 3      | 4     | 2 (tied) |
