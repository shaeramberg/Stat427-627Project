...
set.seed(627)
#Linear kernel
svm.lin<-svm(pass_fail ~ . -G3-five_level-G2-G1, kernel="linear", data=portuguese.data, cross=10)
svm.lin$tot.accuracy

#Polynomial Kernel
svm.pol<-svm(pass_fail ~ . -G3-five_level-G2-G1, kernel="polynomial", data=portuguese.data, cross=10)
svm.pol$tot.accuracy
#Radial kernel
svm.rad<-svm(pass_fail ~ . -G3-five_level-G2-G1, kernel="radial", data=portuguese.data, cross=10)
svm.rad$tot.accuracy
#Sigmoid kernel
svm.sig<-svm(pass_fail ~ . -G3-five_level-G2-G1, kernel="sigmoid", data=portuguese.data, cross=10)
svm.sig$tot.accuracy

```

I used 10-fold cross validation to create 4 initial SVM models with each kernel and a default cost of 1. They all had similar predication accuracy with the best being a linear kernel at around 84.39% accuracy.
...
```{r}
set.seed(627)
svm.tuning <- tune(svm, pass_fail ~ . -G3-five_level-G2-G1, data = portuguese.data, ranges = list(cost = c(0.01, 0.1, 1, 10, 100), kernel = c("linear", "polynomial", "radial", "sigmoid")))
svm.tuning
svm.tuning$best.parameters
svm.tuning$best.model
```

The best model has a cost of 100, uses the linear kernel, and has a CV prediction error of 13.50%.
...
The best kernel for cost 100 is linear with an error of 13.50%.

The best kernel for cost 10 is linearwith an error of 13.69%.

The best kernel for cost 1 is linear with an error of 14.27%.

The best kernel for cost 0.1 is linear with an error of 15.03%.

The best kernel for cost 0.01 is tied with an error of 16.20%.
...
err.tab <- mean(holdout.data$pass_fail != svm.pred)
err.tab
```
...
```{r}
svm.pred2 <- predict(svm.sig, newdata=holdout.data)
err.tab2 <- mean(holdout.data$pass_fail != svm.pred2)
err.tab2
```

Misclassification rate for an untuned sigmoid model is lower than the tuned linear model, at 12.31%.
